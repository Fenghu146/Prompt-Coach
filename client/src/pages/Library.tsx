...
export function Library(){
  ...
  return (
    <div>
      <h1 className="text-xl font-bold text-slate-900">Prompt Library</h1>
      <div className="mt-3 flex flex-wrap gap-2 bg-white border border-slate-200 rounded-xl p-3">
        ...
        <button onClick={load} className="px-4 py-2 bg-slate-900 text-white rounded-lg text-sm">筛选</button>
      </div>
      <div className="mt-4 grid gap-3">
        {rules.length===0 && <p className="text-sm text-slate-500">暂无规则 — 从案例 Learn 并确认后将出现在这里</p>}
        {rules.map(r=>(
          <div key={r.id}>
            <RuleCard rule={r} actions={
              <>
                <button onClick={()=>{
                  setEditId(r.id); setEdit({ title:r.title, promptRule:r.promptRule, experience:r.experience, tags:r.tags.join(", "), confidence:r.confidence });
                }} className="px-3 py-1.5 border border-slate-200 rounded-lg text-sm">编辑</button>
                {r.status==="draft" && <button onClick={async()=>{ setBusy(r.id); await api.patchRule(r.id,{status:"confirmed"}); await load(); setBusy(""); }} disabled={busy===r.id} className="px-3 py-1.5 bg-emerald-600 text-white rounded-lg text-sm">确认</button>}
                {r.status==="confirmed" && <button onClick={async()=>{ setBusy(r.id); await api.patchRule(r.id,{status:"archived"}); await load(); setBusy(""); }} disabled={busy===r.id} className="px-3 py-1.5 border border-slate-200 rounded-lg text-sm">归档</button>}
                <button onClick={async()=>{ setBusy(r.id); await api.deleteRule(r.id); await load(); setBusy(""); }} disabled={busy===r.id} className="px-3 py-1.5 border border-red-200 text-red-600 rounded-lg text-sm">删除</button>
              </>
            }/>
            {editId===r.id && (
              <div className="mt-2 bg-slate-50 border border-slate-200 rounded-lg p-3 space-y-2">
                ...
                <div className="flex gap-2">
                  <button onClick={async()=>{ await api.patchRule(r.id,{ title:edit.title, experience:edit.experience, promptRule:edit.promptRule, tags: edit.tags.split(/[,，\s]+/).filter(Boolean).slice(0,6), confidence: edit.confidence }); setEditId(null); await load(); }} className="px-4 py-2 bg-slate-900 text-white rounded-lg text-sm">保存</button>
                  <button onClick={()=>setEditId(null)} className="px-4 py-2 border border-slate-200 rounded-lg text-sm">取消</button>
                </div>
              </div>
            )}
          </div>
