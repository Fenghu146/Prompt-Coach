...
export function NewPrompt(){
  ...
  async function load(){ setCases(await api.listCases()); }
  useEffect(()=>{ load(); },[]);
  ...
  async function submit(e:React.FormEvent){
    ...
    try{
      const tags = form.tags.split(/[,，\s]+/).map(s=>s.trim()).filter(Boolean).slice(0,6);
      const c = await api.createCase({ title: form.title||form.problem.slice(0,40)||"未命名案例", domain: form.domain, problem: form.problem, originalPrompt: form.originalPrompt, context: form.context||undefined, aiResult: form.aiResult||undefined, tags });
      nav(`/cases/${c.id}`);
    }catch(ex:unknown){ setErr((ex as Error).message); } finally{ setLoading(false); }
  }

  return (
    <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
      <div className="lg:col-span-3">
        <h1 className="text-xl font-bold text-slate-900">New Prompt</h1>
        <p className="text-sm text-slate-600 mt-1">记录真实问题与原始 Prompt，默认面向嵌入式 Debug。提交后在详情页执行 Improve。</p>
        <form onSubmit={submit} className="mt-4 space-y-3 bg-white border border-slate-200 rounded-xl p-4">
          ...
          <textarea value={form.originalPrompt} onChange={e=>setForm({...form,originalPrompt:e.target.value})} placeholder="Original Prompt * — 你原本写下的提示词" rows={3} required className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm"/>
          ...
          <button disabled={loading} className="w-full py-2.5 bg-slate-900 text-white rounded-lg text-sm font-medium hover:bg-slate-800 disabled:opacity-50">{loading?"提交中…":"创建案例并去优化"}</button>
        </form>
      </div>
      <div className="lg:col-span-2">
        <h2 className="font-semibold text-slate-900">最近案例</h2>
        <div className="mt-3 space-y-2">
          {cases.length===0 && <p className="text-sm text-slate-500">暂无案例 — 创建第一条后将出现在这里</p>}
          {cases.map(c=>(
