...
export function Settings(){
  ...
  async function save(){
    ...
    try{
      ...
      setS(cur); setForm(f=>({...f,apiKey:""})); setMsg("已保存");
    }catch(e:unknown){ setMsg((e as Error).message); } finally{ setBusy(""); }
  }
  async function test(){
    setBusy("test"); setMsg("");
    try{
  ...
  return (
    <div className="max-w-2xl">
      ...
      <p className="text-sm text-slate-600 mt-1">配置 OpenAI 兼容接口（支持 chat/completions 与 responses），前端设置页输入；未配置时自动使用内嵌免配置模型。</p>
      <div className="mt-4 bg-white border border-slate-200 rounded-xl p-4 space-y-3">
        ...
        <div>
          <label className="text-xs font-medium text-slate-700">apiKey {s.hasApiKey && <span className="text-slate-500">（已配置：{s.apiKeyMasked}，留空不改）</span>}</label>
        ...
        <div className="flex gap-2">
          <button onClick={save} disabled={busy==="save"} className="px-4 py-2 bg-slate-900 text-white rounded-lg text-sm disabled:opacity-50">{busy==="save"?"保存中…":"保存"}</button>
          <button onClick={test} disabled={busy==="test"} className="px-4 py-2 border border-slate-200 rounded-lg text-sm disabled:opacity-50">{busy==="test"?"测试中…":"测试连接"}</button>
        </div>
        {msg && <p className="text-sm whitespace-pre-wrap p-3 rounded-lg border bg-slate-50 border-slate-200">{msg}</p>}
        <p className="text-xs text-slate-500">Key 仅存后端 server/data/settings.json，不回显明文；未配置或测试失败时 Improve/Generate 自动回退内嵌模型并提示。</p>
      </div>
    </div>
  );
}
