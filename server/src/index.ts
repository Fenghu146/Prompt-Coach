...
async function ensureSeed() {
  const casesPath = dataPath(DATA_DIR, "cases");
  const rulesPath = dataPath(DATA_DIR, "rules");
...
app.use("/api/cases", casesRouter(DATA_DIR));
app.use("/api/rules", rulesRouter(DATA_DIR));
app.use("/api/generate", generateRouter(DATA_DIR));
