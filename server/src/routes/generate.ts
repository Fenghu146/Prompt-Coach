...
export function generateRouter(dataDir: string){
  ...
  router.post("/", async (req,res)=>{
    ...
    const retrievedWithReasons = retrieveWithReasons(rules, { query: parsed.data.problem, domain: parsed.data.domain, tag: parsed.data.tags?.[0], limit: 7, onlyConfirmed: true });
    const retrieved = retrievedWithReasons.map(x=>x.rule);
    ...
    let provider =
      settings.model === "__embedded__"
        ...
        : settings.apiKey && settings.apiKey.trim().length > 10
          ...
          : (new LocalEmbeddedProvider() as never);
    let result; let usedFallback=false; let fallbackReason="";
    if (settings.model==="__embedded__") fallbackReason="当前为内嵌模型（离线/零 Token）";
    try { result = await (provider as unknown as { generate:(x:never)=>Promise<never> }).generate({ problem: parsed.data.problem, domain: parsed.data.domain, context: parsed.data.context, retrievedRules: retrieved } as never); }
      ...
      if ((provider as {name:string}).name!=="local-embedded"){
        fallbackReason = (e as Error).message?.slice(0,200) || "OpenAI 调用失败";
    ...
    res.json({ prompt: r.prompt, retrievedRules: retrievedWithReasons.map(x=>({ ...x.rule, _score:x.score, _reasons:x.reasons })), provider: r.provider, usedEmbeddedFallback: usedFallback, fallbackReason });
  });
  return router;
}
